...
import {
  ...
  Search, AlertCircle, TrendingDown, CheckCircle2, ArrowRight, RotateCcw
} from 'lucide-react';
    ...
    if (this.state.hasError) {
      return (
        <div className="min-h-screen bg-[#080C14] text-slate-100 flex flex-col items-center justify-center p-6 text-center space-y-4 font-sans">
          ...
          <h1 className="text-xl font-black text-white">Dashboard Encountered a Runtime Error</h1>
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2.5 bg-[#162032] hover:bg-[#1f2d47] border border-[#2b3a58] text-slate-200 text-xs font-bold rounded-xl flex items-center gap-2 transition-all cursor-pointer"
            >
              Choose Different Site
            </button>
            <button
              onClick={() => {
...
export const BuildingReportsModule: React.FC = () => {
  ...
  const totalHoldingValuation = productAuditReport.reduce((sum, p) => sum + (p.holdingValue || 0), 0);
  const totalConsumedExpenditure = productAuditReport.reduce((sum, p) => sum + (p.consumedValue || 0), 0);
  const totalInwardProcurement = productAuditReport.reduce((sum, p) => sum + (p.inwardValue || 0), 0);
                ...
                filteredProducts.map((p) => (
                  <tr key={p?.id} className="hover:bg-[#121c33]/50 transition-colors">
                    ...
                    <td className="py-3.5 px-5 text-right font-mono font-black text-sm text-emerald-400">
                      ₹{(p?.holdingValue || 0).toLocaleString('en-IN')}
                    </td>
                  </tr>
                ))
              )}
    ...
    <div className="space-y-6 font-sans text-slate-100">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Real-time telemetry tracking stockouts, safety buffer limits, and rapid outward consumption rates.
          </p>
        </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-[#1E293B]/60 text-slate-200">
              {filteredProducts.length === 0 ? (
                ...
                filteredProducts.map((p) => {
                  const isLow = Number(p?.currentStock || 0) <= Number(p?.minThreshold || 0);
                  const assetValue = Number(p?.currentStock || 0) * Number(p?.unitCost || 0);
                  ...
                  return (
                    <tr key={p?.id} className="hover:bg-[#121c33]/50 transition-colors">
                      ...
                      <td className="py-4 px-5 text-center whitespace-nowrap">
                        ...
                        >
                          {Number(p?.currentStock || 0).toLocaleString('en-IN')} {p?.unit}
                        </span>
                      </td>
...
// ==========================================
// Header Component with Safe Fallbacks
// ==========================================
        ...
        <div className="relative">
          ...
          >
            ...
            <span className="font-mono text-blue-400 truncate max-w-[120px] sm:max-w-[200px]">
              {currentSiteSheet?.siteName || currentSiteSheet?.name || 'Active Site'}
            </span>
...
// ==========================================
// Sidebar Component with Dynamic Alert Count
// ==========================================
...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Dynamic live alert count computation
...
export const AppContent: React.FC = () => {
  ...
  // Gatekeeper: only open SiteSelectionPage when no site is chosen
